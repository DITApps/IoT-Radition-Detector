...
//#define WIFI_SSID "AndroidHotspot3270"                                             // input your home or public wifi name 
//#define WIFI_PASSWORD "a0519690"   
...
void setup() {
  ...
  Serial.println("");configTime(9 * 3600 + 40, 0, "pool.ntp.org", "time.nist.gov");
  Serial.println("\nWaiting for time");
  while (!time(nullptr)) {
    Serial.print(".");
    delay(1000);
...
{ 
  ...
  getTextTime(now);
  // Firebase에 전
  Firebase.pushString("/nowTime", ctime(&now));    
...
void Print_Result(int cmd) {
  ...
  while (ss.available() > 0)
  {
    if (gps.encode(ss.read()))
    {
      if (gps.location.isValid())
      {
        ...
        Firebase.pushFloat("/GPS/lat", _lat);  
        Firebase.pushFloat("/GPS/lng", _lng);  
  ...
  switch (cmd) {
    ...
    case 0xB2:
      ...
      Firebase.pushFloat("/value10min", val_10min);   
    ...
    case 0xB3:
      ...
      // Firebase로 1분당 측정값 보내기
      Firebase.pushFloat("/value1min", val_1min);   
